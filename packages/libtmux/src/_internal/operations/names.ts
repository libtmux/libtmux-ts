 ...
 * name means three things. `:` and `.` also split a target, so an ordinary
 * `-t name` lookup misreads a name holding one as two fields on every
 * version; addressing it again needs a spelling ordinary lookups do not use.
 * Python libtmux refuses the same two through `session_check_name`.
 */
