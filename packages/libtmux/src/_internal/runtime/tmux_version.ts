...
// A release candidate names the release it precedes: 3.8 ships its candidates
// as `3.8-rc`, and older releases numbered them `3.0-rc3`. tmux never numbers
// one `-rc0`.
const releaseCandidate = /-rc(?:[1-9]\d*)?$/u;
 ...
 *   `3.8-rc` dumps the v2 JSON layout that arrived in 3.8, so a gate asking
 *   whether this tmux has a 3.8 feature has to answer yes.
...
export function parseTmuxVersion(raw: string): TmuxVersion {
  ...
  // rather than as alternatives: a string carrying two of these markers is
  // otherwise classified on one and parsed on another. No tmux emits that
  // shape, but a version this file accepts must be one it can also rank.
