...
): readonly (readonly [name: string, criteriaType: string])[] {
  return criteriaFieldsForModel(fields, model)
    .map(({ criteriaName, domain }) => {
      const criteriaType = criteriaTypeForDomain[domain];
      if (criteriaType === undefined) {
        throw new Error(`criteria field ${criteriaName} has unknown domain ${domain}`);
      }
      return [criteriaName, criteriaType] as const;
    })
...
export function renderGeneratedWhereTypeRegion(
  ...
  for (const model of criteriaModels) {
    ...
    for (const [field, criteriaType] of generatedWhereFieldEntries(fields, model)) {
      lines.push(`  readonly ${field}?: ${criteriaType};`);
    }
    for (const relation of CRITERIA_RELATIONS_V1[model]) {
...
function aliasesForModel(
  ...
  const tokens = fields
    .filter(({ scope }) => scope === model || scope === "universal")
    .map(({ token }) => token);
...
/** Said once. Three of these files did not carry it, which is friction. */
const GENERATED_BANNER = "// Generated by scripts/generate-formats.ts. Do not edit.";

function renderFieldAliasesSource(fields: readonly GeneratedFormatField[]): string {
  const typeNames = ALIAS_MODELS.map(
    (model) => `${model[0]!.toUpperCase()}${model.slice(1)}AliasMap`,
  );
  const lines = [
    GENERATED_BANNER,
    "",
...
function renderAliasMapTypes(fields: readonly GeneratedFormatField[]): string[] {
  const lines: string[] = [];
  for (const model of ALIAS_MODELS) {
    const typeName = `${model[0]!.toUpperCase()}${model.slice(1)}AliasMap`;
...
async function readValueTypes(tokens: ReadonlySet<string>): Promise<ReadonlyMap<string, string>> {
  ...
  if (!isRecord(parsed) || !exactKeys(parsed, ["note", "types"]) || !isRecord(parsed.types)) {
    throw new Error("format value-type fixture must contain exactly note and types");
  }
...
/** The generated field vocabulary and decoded value types exposed to consumers. */
function renderPublicFieldTypesSource(
  ...
  return `${[
    ...
    "};",
    "",
    ...renderAliasMapTypes(fields),
  ].join("\n")}`;
}

/** The runtime lookup tables backed by the public generated type authority. */
...
function renderPrivateFieldNamesSource(): string {
...
export async function generateFormats(options: GenerateFormatsOptions): Promise<void> {
  ...
  const expected = new Map<string, string>([
    [join(options.outputDirectory, "format_field_names.ts"), renderPrivateFieldNamesSource()],
    [
      join(options.outputDirectory, "format_fields.ts"),
      `${GENERATED_BANNER}\n\n${generated["format_fields.ts"] ?? ""}`,
    ],
    [
      join(options.outputDirectory, "..", "field_types.ts"),
      renderPublicFieldTypesSource(
        generated["format_field_names.ts"] ?? "",
        valueTypes,
        vocabulary,
      ),
    ],
    [join(options.outputDirectory, "field_types.ts"), renderFieldTypesSource(valueTypes)],
    [join(options.outputDirectory, "field_aliases.ts"), renderFieldAliasesSource(vocabulary)],
    [
