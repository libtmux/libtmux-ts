 ...
 * created, but a doc example is free to write `new Server()` itself. Several
 * TSDoc examples on `Server` do, and so does the README quickstart. Those
 * resolve against `TMUX_TMPDIR` rather than a run root this package tracks, so
 * nothing reaps them.
 *
 ...
 * session — after the process that started it exited. Both
 * check-readme-runs.ts and check-symbol-runs.ts point `TMUX_TMPDIR` at a
 * directory nothing else touches, so every socket found under it belongs to a
 * doc example, and `kill-server` on each is safe.
 */
