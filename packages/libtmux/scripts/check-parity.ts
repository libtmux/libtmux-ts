...
function verifyManualProvenance(
  ...
  for (const behavior of manifest.observableBehaviors) {
    ...
    // As with a public symbol, a behavior recorded as deliberately not ported
    // has no evidence to give, whatever it would otherwise have required.
...
// The test helpers stay untracked: `./test` is not a published entrypoint, so
// nothing in the package can satisfy a claim pointing at one.
...
function validateSymbolPolicies(manifest: ParityManifest, expected: PublicSymbol[]): string[] {
  ...
  for (const entry of manifest.publicSymbols) {
    ...
    // The policy is derived against the status this row actually records, so a
    // row can say "not ported" without also having to produce a test for it.
    const expectedApplicability =
      expectedEntry &&
