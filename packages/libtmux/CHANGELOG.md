...
as a deprecated alias for the renamed `QueryValidationReason`.
...
Change the import path — the function is unchanged.
...
default. A timeout rejects with `kind: "timeout"`.
...
`TextDecoder` if you called `toString()` on one.
...
real `AbortSignal` already satisfies.
...
place of the deprecated `Server.raiseIfDead()` alias.
...
too, and any rejection an observer throws is swallowed.
...
container, cron, or an MCP client that curates what it passes on.
...
or `<tag>-master`) still satisfies any minimum.
...
3.5 and JSON layouts before 3.8 throw `VersionTooLowError`.
...
naming every preset it could mean.
...
same error, carrying its `code`, `delivery` and `kind`.
...
the same fact that decides whether `teardown` tools are offered.
...
annotations, and `tmux://capabilities` is what tells them apart.
...
`wait_for_text` or other live read holds one open while it runs.
