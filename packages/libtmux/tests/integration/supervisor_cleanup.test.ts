 ...
 * actually needs.
 */
async function readMarker<T>(path: string, timeoutMs = 30_000): Promise<T> {
...
/**
 * Wait for a worker to create its marker.
 *
 * Bounded for liveness rather than speed: what it waits on is a `bun` process
 * starting, which on a busy machine takes far longer than the second this used
...
async function waitForPath(path: string, timeoutMs = 30_000): Promise<void> {
  ...
  for (;;) {
    try {
      ...
      if (Date.now() > deadline) throw new Error(`path was not created: ${path}`);
  ...
  for (const signal of ["SIGTERM", "SIGKILL"] as const) {
    test(`reaps a fixture abandoned by a worker killed with ${signal}`, async () => {
      ...
      try {
        ...
        const state = await readMarker<{ daemonPid: number }>(marker);
        child.kill(signal);
...
describe("supervisor status and signal semantics", () => {
  ...
  test("standalone reaper cleans only after a SIGKILLed supervisor owner is dead", async () => {
    ...
    try {
      await waitForPath(marker);
      const state = await readMarker<{ daemonPid: number; workerPid: number }>(marker);
  ...
  test("progresses after SIGKILL when a descendant keeps the child pipes open", async () => {
    ...
    try {
      await waitForPath(marker);
      holderPid = (await readMarker<{ holderPid: number }>(marker)).holderPid;
