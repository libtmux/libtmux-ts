 ...
 */
async function expectNoReservations(runRoot: string, before: ReadonlySet<string>): Promise<void> {
  let added: string[] = [];
...
/**
 * Wait for a probe subprocess to publish its marker.
 *
 * The probe writes this only after the deadline it is driving to expiry has
 * passed, so the bound here has to clear that deadline with room to spare —
 * it says the probe never got there at all, not that it was slow.
 */
async function waitForMarker(path: string): Promise<void> {
  const deadline =
    performance.now() + deadlineMs(CONTROL_REGISTRATION_DEADLINE_MS) * 4 + deadlineMs(2_000);
  while (performance.now() < deadline) {
    try {
      // eslint-disable-next-line no-await-in-loop -- the subprocess marker is observed within one monotonic bound.
      await access(path);
      return;
    } catch {
...
describe("supervised TestServer", () => {
  ...
  test("authenticates a valid launch frame from a nonzero result before cleanup", async () => {
    ...
    try {
      ...
      await waitForProcessExit(Number(rawPid));
      await waitForPathAbsent(socketPath!);
      await expectNoReservations(runRoot, before);
    } finally {
      try {
        const [socketPath] = (await readFile(marker, "utf8")).trim().split("\t");
        if (socketPath !== undefined) {
  ...
  for (const mode of ["partial-open", "dispose"] as const) {
    test(`does not retain a Bun process through a losing ControlMode ${mode} timer`, async () => {
      ...
      try {
        ...
        const closed = await closeWithin(child, bound);
