...
/**
 * Cancellation belongs to every operation that takes options, not to whichever
 * one happened to get it first. An asymmetric API is worse than a uniform gap:
 * a caller learns the rule on one method and it fails on the next.
 */
declare const cancelPane: Pane;
declare const cancelWindow: Window;
declare const cancelSession: Session;
declare const cancelServer: Server;
...
export const cancellable: readonly Promise<unknown>[] = [
  ...
  cancelPane.joinTo("@1", { signal: abort }),
  cancelPane.setOption("a", "b", { signal: abort }),
