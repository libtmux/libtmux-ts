...
describe("generated Where contract", () => {
  test("writes exact relation metadata and only the delimited cyclic interfaces", async () => {
    ...
    // Sized for what it does: this test reads the generated region through
    // TypeScript's Go API, retried up to three times when a contended run
    // reports a cancellation. Three compiler spawns do not fit in Bun's default
