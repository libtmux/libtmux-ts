...
describe("Selection collection contract", () => {
  ...
  test("maps members to values, leaving the selection domain", async () => {
    const harness = await createSessionHarness(["alpha", "beta"]);
    const selection = createProjectedSelection("session", harness.values, harness.projection);

    // An array, not a Selection: the results are no longer tmux objects and
    // carry no identity to filter or traverse from.
    expect(selection.map((session) => session.name)).toEqual(["alpha", "beta"]);
    expect(selection.map((session, index) => `${String(index)}:${session.name ?? ""}`)).toEqual([
      "0:alpha",
      "1:beta",
    ]);
    expect(Array.isArray(selection.map((session) => session))).toBe(true);
    expect(() => selection.map(undefined as never)).toThrow(TypeError);
  });
