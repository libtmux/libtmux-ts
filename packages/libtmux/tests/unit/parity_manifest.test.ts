...
describe("Python 0.62.0 parity manifest", () => {
  ...
  test("activates the pure format symbols while deferring executor-owned parity", async () => {
    ...
    // These two are the only neo symbols carved out of the module's blanket
    // "not ported", because the fetching they describe is exactly what
    // `snapshot()` and `refresh()` do. The carve-out exists so they can carry
    // evidence rather than inherit a not-applicable lane.
    for (const python of ["libtmux.neo.fetch_obj", "libtmux.neo.fetch_objs"]) {
      const row = manifest.publicSymbols.find((entry) => entry.python === python);
      expect(row).toMatchObject({ status: "adapted" });
      expect(row?.realTmuxScenario).not.toBeNull();
      expect(row?.typescriptSymbols.length).toBeGreaterThan(0);
    }
    // The lenient empty-Selection behavior these describe is deliberately not
    // ported: an unreachable server raises here instead.
  ...
  test("records the accepted behavior, extension, and exclusion sets", async () => {
    ...
    for (const symbol of manifest.publicSymbols) {
      if (
        ["method", "compatibility-alias", "property"].includes(symbol.kind) &&
        // A symbol recorded as deliberately not ported has no evidence to give
        // in any lane, whatever its kind.
        symbol.status !== "unsupported"
      ) {
        expect(symbol.evidenceApplicability.realTmuxScenario).toBe("required");
      }
    }
    for (const python of requiredRealTmuxSymbols) {
      const symbol = manifest.publicSymbols.find((entry) => entry.python === python);
      // A row recorded as deliberately not ported has no evidence to give in
      // any lane, so the requirement only binds a row that claims a port.
      if (symbol?.status === "unsupported") continue;
      expect(symbol?.evidenceApplicability.realTmuxScenario).toBe("required");
    }
    expect(
  ...
  const invalidCases: Array<[string, (manifest: ParityManifest) => void, string]> = [
    ...
    [
      ...
      (manifest) => {
        // A behavior that genuinely claims a port and genuinely needs a real
        // tmux run, so removing its evidence is the situation this names.
        const behavior = manifest.observableBehaviors.find(({ id }) => id === "cmd-protocol.call")!;
    ...
    [
      ...
      (manifest) => {
        // Clearing the recorded evidence too, so the run reaches the
        // applicability comparison rather than stopping at the earlier guard
        // that a not-applicable lane must carry no evidence.
        ...
        manifest.observableBehaviors[0]!.realTmuxScenario = null;
      },
