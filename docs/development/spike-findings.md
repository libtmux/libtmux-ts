...
## The supervisor suite was not flaky

Date: 2026-08-13.

### Question

The integration suite failed intermittently — measured at various points as one
run in five, one in ten, three in twelve. Three separate fixes went in for it:
bounded waits for reclaimed resources, tolerating a daemon that exits mid-read,
and sharing those waits across suites. It kept coming back, and every report of
it called it pre-existing and timing-sensitive.

### Finding

The machine was carrying five processes at 100% of a core each, for fifteen
hours, with a load average of 22 on twenty cores.

They came from the fixtures that verify signal escalation. Those spawn a shell
that ignores `TERM` and never exits, and it was held open with
`while :; do :; done`. A wrapper that outlives its test — an interrupted run, a
cleanup that loses a race — then spins until somebody notices. Nobody did.

Every flake measurement taken during that period was measured against a machine
the measuring process had itself been loading.

### Decision

Hold with `while :; do sleep 30; done`. It stays alive and still ignores `TERM`,
so the escalation tests are unchanged, and a leaked wrapper costs nothing.

After clearing the spinners: 23 consecutive integration runs and 16 consecutive
unit runs, no failures. The three earlier fixes were each correct on their own
terms — an instantaneous assertion on an asynchronous reclamation is a real
defect — but they were not what made the suite unreliable.

### What to do differently

Check the load average before characterising a flake. A rate that moves between
measurements is describing the machine, not the code.

## Validator bakeoff: what a dependency was buying

Date: 2026-08-13.

### Question

The package had one runtime dependency. Zero is a property downstream teams
weigh directly — nothing to audit, pin, or deduplicate — so the question was
what Zod was actually providing, and whether a validator covering only this
package's needs could provide it.

### Contenders

Three, each implementing the same three real schemas — the identity strings,
the 178-key format row, and the versioned where-document — and measured on the
same six failing inputs.

| Variant                  | Bundled | Message on a failed regex                     | Reports the value |
| ------------------------ | ------: | --------------------------------------------- | ----------------- |
| Zod classic              |  280 KB | `Invalid string: must match /^%\d+$/u`        | no                |
| `zod/mini`               |   17 KB | `Invalid input`                               | no                |
| `zod/mini` + `en` locale |  160 KB | `Invalid string: must match /^%\d+$/u`        | no                |
| In-house, functional     |  2.8 KB | `expected a pane id like %0, received "nope"` | yes               |
| In-house, chainable      |  3.8 KB | identical to functional                       | yes               |

### Finding

`zod/mini` reaches its size by dropping the message machinery: every failure,
whatever the cause, answers `"Invalid input"`. The structured fields survive,
so a program can still branch on `code`, but a human reading a log learns
nothing. Restoring the messages means loading a locale, and that costs more
than nine times what Mini saved.

Both in-house spikes produced byte-identical issues across all six inputs, so
the choice between them was purely surface. Chainable won for 38 lines and
1 KB: the real call sites chain — `string().min(1).refine(...)`,
`string().regex(...).nullable()` — and the functional form nests those
inside-out.

The in-house issues carry one field Zod's never do: the value that failed,
rendered and capped at 60 characters. Debugging a tmux frame, that is the
field you want.

### Decision

Ship the chainable in-house validator; keep Zod as a devDependency so the
bakeoff stays reproducible. A realistic consumer bundle went from 416 KB to
140 KB — the size of this package's own code, with the validator not
registering at that granularity.

### What this does not buy

Zod is still the right answer for anything with coercion, transforms, async
refinement, or schema inference across a large surface. This package validates
three shapes, none of which needs any of that. The trade is only correct
because the requirement is small — and now that it is ours, it is ours to keep
correct, which is what tests/unit/validate.test.ts is for.
