...
directly. Criteria produced by `parseLegacyWhere()` also report the lowered
...
The exported `parseLegacyWhere(model, input)` is this adapter's only public
...
export function parseLegacyWhere<M extends "session" | "window">(
  model: M,
  input: unknown,
